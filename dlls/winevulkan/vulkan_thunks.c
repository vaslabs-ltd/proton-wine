...
#endif /* USE_STRUCT_CONVERSION */
{
    ...
    out->sType = in->sType;
    out->pNext = in->pNext;
    out->semaphore = in->semaphore ? wine_semaphore_from_handle(in->semaphore)->semaphore : VK_NULL_HANDLE;
    out->handleType = in->handleType;
}
...
static inline VkSemaphoreSubmitInfo *convert_VkSemaphoreSubmitInfo_array_win_to_host(const VkSemaphoreSubmitInfo *in, uint32_t count)
{
    ...
    for (i = 0; i < count; i++)
    {
        out[i].sType = in[i].sType;
        out[i].pNext = in[i].pNext;
        out[i].semaphore = in[i].semaphore ? wine_semaphore_from_handle(in[i].semaphore)->semaphore : VK_NULL_HANDLE;
...
static NTSTATUS wine_vkAcquireNextImageKHR(void *args)
{
    ...
    return params->device->funcs.p_vkAcquireNextImageKHR(params->device->device, ((struct VkSwapchainKHR_T *)(uintptr_t) (params->swapchain))->swapchain, params->timeout, params->semaphore ? wine_semaphore_from_handle(params->semaphore)->semaphore : VK_NULL_HANDLE, params->fence ? wine_fence_from_handle(params->fence)->fence : VK_NULL_HANDLE, params->pImageIndex);
}
...
{
    return device->funcs.p_vkGetSemaphoreCounterValue(device->device, wine_semaphore_from_handle(semaphore)->semaphore, pValue);
}

VkResult thunk_vkGetSemaphoreCounterValueKHR(VkDevice device, VkSemaphore semaphore, uint64_t *pValue)
{
    return device->funcs.p_vkGetSemaphoreCounterValueKHR(device->device, wine_semaphore_from_handle(semaphore)->semaphore, pValue);
}
...
uint64_t wine_vk_unwrap_handle(VkObjectType type, uint64_t handle)
{
    ...
    case VK_OBJECT_TYPE_SEMAPHORE:
        return (uint64_t) wine_semaphore_from_handle(handle)->semaphore;
